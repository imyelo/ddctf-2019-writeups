...
接下来要做的事情是在第一份文件里找出再次渲染时没有发生变化的片段，并往这些片段里填入目标字符串（这里我用 `<?phpinfo()?>` 尝试）：
...
虽然所有文件在视觉上都发生了明显的变化，但经过逐个上传，发现其中 [2.jpg](./vendors/inject-output/2.jpg) 和 [4.jpg](./vendors/inject-output/4.jpg) 注入的片段在渲染后没有消失：
